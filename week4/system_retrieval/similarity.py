...
class knnDistance(SimilaritySearch):
    def calculate(self, aa, bb):
        thr = 0
        ...
        if des1 is not None and des2 is not None:
            ...
            if len(good) > 3 and ransac:
                ...
                M, mask = cv.findHomography(src_pts, dst_pts, cv.RANSAC, ransacReprojThreshold=thr)

                # Count inliers (matches that are consistent with the estimated homography)
                inliers = mask.ravel().tolist().count(1)


            # if we dont have enough, we consider that the second image is not in the db
                if inliers < thr:
                    return -1
                else:
                    return inliers
...
class L1kpDistance(SimilaritySearch):
    def calculate(self, aa, bb):
        thr = 0
...
class L2kpDistance(SimilaritySearch):
    def calculate(self, aa, bb):
        thr = 0
...
class HammingDistancekp(SimilaritySearch):
    def calculate(self, aa, bb):
        thr = 0
...
class FlannDistance(SimilaritySearch):
    def calculate(self, aa, bb):
        thr = 0
